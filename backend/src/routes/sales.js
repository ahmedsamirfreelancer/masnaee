...
router.post('/', authorize('sales.create', '*'), [
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO sales_orders (order_number, customer_id, order_date, subtotal, discount, tax_rate, tax_amount, total_amount, notes, status, payment_status, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'draft', 'unpaid', ?)`,
      [order_number, customer_id, order_date || new Date(), subtotal, disc, tax_rate || 0, tax, total, notes, req.user.id]
    );
    ...
    for (const item of items) {
      ...
      await conn.query(
        `INSERT INTO sales_order_items (sales_order_id, product_id, quantity, unit_price, line_total)
         VALUES (?, ?, ?, ?, ?)`,
        [orderId, item.product_id, item.quantity, item.unit_price, line_total]
      );
    }
...
router.put('/:id', authorize('sales.edit', '*'), async (req, res, next) => {
  ...
  try {
    ...
    await conn.query(
      `UPDATE sales_orders SET customer_id=?, subtotal=?, discount=?, tax_rate=?, tax_amount=?, total_amount=?, notes=? WHERE id=?`,
      [customer_id || order[0].customer_id, subtotal, disc, tax_rate || 0, tax, total, notes, req.params.id]
    );
    ...
    if (items) {
      ...
      for (const item of items) {
        ...
        await conn.query(
          `INSERT INTO sales_order_items (sales_order_id, product_id, quantity, unit_price, line_total)
           VALUES (?, ?, ?, ?, ?)`,
          [req.params.id, item.product_id, item.quantity, item.unit_price, line_total]
        );
      }
    }
...
router.put('/:id/ship', authorize('sales.edit', '*'), async (req, res, next) => {
  try {
    const [result] = await db.query(
      `UPDATE sales_orders SET status = 'shipped', shipped_date = NOW() WHERE id = ? AND status = 'confirmed'`, [req.params.id]
    );
  ...
  try {
    ...
    for (const item of items) {
      ...
      await conn.query(
        `INSERT INTO inventory_movements (item_type, item_id, movement_type, quantity, reference_type, reference_id, notes, created_by)
         VALUES ('product', ?, 'out', ?, 'sales', ?, 'تسليم أمر بيع', ?)`,
    ...
    const [jeResult] = await conn.query(
      `INSERT INTO journal_entries (entry_number, entry_date, reference_type, reference_id, description, is_posted, created_by)
       VALUES (?, NOW(), 'sales', ?, ?, TRUE, ?)`,
      [generateNumber('JE', order.id), req.params.id, `تسليم أمر بيع ${order.order_number}`, req.user.id]
    );
    ...
    await conn.query(
      `UPDATE sales_orders SET status = 'delivered', delivered_date = NOW() WHERE id = ?`, [req.params.id]
    );
    await conn.commit();
  ...
  try {
    ...
    if (order.status === 'delivered') {
      ...
      for (const item of items) {
        ...
        await conn.query(
          `INSERT INTO inventory_movements (item_type, item_id, movement_type, quantity, reference_type, reference_id, notes, created_by)
           VALUES ('product', ?, 'in', ?, 'sales_cancel', ?, 'إلغاء أمر بيع', ?)`,
          [item.product_id, item.quantity, req.params.id, req.user.id]
        );
      }
    }
...
router.post('/returns', authorize('sales.create', '*'), [
  ...
  try {
    ...
    for (const item of items) {
      ...
      await conn.query(
        `INSERT INTO inventory_movements (item_type, item_id, movement_type, quantity, reference_type, reference_id, notes, created_by)
         VALUES ('product', ?, 'in', ?, 'sales_return', ?, ?, ?)`,
        [item.product_id, item.quantity, sales_order_id, reason || 'مرتجع مبيعات', req.user.id]
      );
    }

    const [result] = await conn.query(
      `INSERT INTO sales_returns (sales_order_id, return_date, total_amount, reason, created_by)
       VALUES (?, NOW(), ?, ?, ?)`,
      [sales_order_id, totalReturn, reason, req.user.id]
    );
